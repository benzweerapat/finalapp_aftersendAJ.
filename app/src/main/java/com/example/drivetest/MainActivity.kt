...
//
    ...
    @Volatile private var isSavingWifiZip: Boolean = false
...
// โดยใช้ชื่อไฟล์ตามรูปแบบ: Session_<id>_neighbor_<TECH>_<timestamp>.csv
    ...
    fun resetIndoorSelectionsForNewFloorPlan() {
        hasSelectedStartFloor = false
        hasSelectedFloorHeight = false
        isGroundSet = false

        // ถ้าต้องการให้บังคับ set ground ใหม่จริง ๆ แนะนำล้าง reference ด้วย
        referencePressure = -1f
        referenceGpsAltitude = null

        updateFloorButtonLabel()
        refreshFloorHeightButtonLabel()

        // ให้ Indoor panel รีเฟรช label ด้วย (ปลอดภัย)
    ...
    private fun updateTechButtonUi() {
        ...
        btn.text = if (currentTech == CurrentTech.WIFI) "WiFi" else "Cell"
    }
    ...
    private fun renderCurrentScreen() {
        ...
        updateTechButtonUi()       // ✅ ให้ btnDriveMode เป็น WiFi/Cell ถูกเสมอ
    }
    ...
    private fun setupButtons() {
        ...
        updateTechButtonUi()   // ✅ เพิ่มบรรทัดนี้
        updateUnifiedSurveyButtonUi()

        btnDriveMode.setOnClickListener { view ->
            ...
            // ใช้เมนูเดิม scan_menu ได้เลย (menu_wifi / menu_cellular)
        ...
        findViewById<ImageView>(R.id.btnMenu).setOnClickListener {
            ...
            val modes = arrayOf("GPS/GNSS", "Floor plan")

            androidx.appcompat.app.AlertDialog.Builder(this)
                .setTitle("Select Mode")
                .setItems(modes) { _, which ->
                    val newMode = if (which == 0) DriveMode.GNSS else DriveMode.FLOOR_PLAN

                    // ถ้าโหมดไม่เปลี่ยน ไม่ต้องทำอะไร
                    if (newMode == currentDriveMode) return@setItems
        ...
        scanBtn.setOnClickListener {
            ...
            when (fragment) {
                ...
                is WifiFragment -> {
                    if (isSavingWifiZip) {
                        toast("Still saving previous Wi-Fi CSV. Please wait")
                        return@setOnClickListener
                    }
                    ...
                    if (!isRecordingWifiCsv) {
                        ...
                        val capturedSsid = fragment.getCurrentSsid()
                        val capturedSessionId = currentWifiSessionId
                        val servingRowsSnapshot = wifiCsvBuffer.toList()
                        val neighborRowsSnapshot = wifiNeighborCsvBuffer.toList()
                        wifiCsvBuffer.clear()
                        wifiNeighborCsvBuffer.clear()
                        ...
                        isSavingWifiZip = true
                        thread(start = true, name = "wifi-zip-writer") {
                            val saveError = try {
                                if (capturedSessionId != null) {
                                    saveGnssZipForWifi(
                                        ssid = capturedSsid,
                                        sessionId = capturedSessionId,
                                        servingRows = servingRowsSnapshot,
                                        neighborRows = neighborRowsSnapshot
                                    )
                                }
                                null
                            } catch (e: Exception) {
                            ...
                            runOnUiThread {
                                isSavingWifiZip = false
                                if (saveError == null) {
                                    toast("All CSVs saved")
                                } else {
                                    toast("Save error: ${saveError.message}")
                                }
                            }
                        }
                    }
...
// ให้เรียกฟังก์ชัน saveWifiNeighborCsv() ต่อท้าย saveWifiCsv()
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                val cv = ContentValues().apply {
                    ...
                    put(MediaStore.MediaColumns.RELATIVE_PATH, "Download/DriveTest_GNSS_Floorplan/${currentDriveMode.folderName}/$subDir")
                ...
                val dir = File(
                    ...
                    "DriveTest_GNSS_Floorplan/${currentDriveMode.folderName}/$subDir"
                )
    ...
    private fun saveGnssZipForWifi(
        ssid: String,
        sessionId: Int,
        ...
        saveZipToDownloads(
            ...
            subDir = "Wifi",
            entries = listOf(
                servName to csvBytes(wifiCsvHeader, servingRows),
                neiName to csvBytes(wifiNeighborHeader, neighborRows)
            )
        )
    }
