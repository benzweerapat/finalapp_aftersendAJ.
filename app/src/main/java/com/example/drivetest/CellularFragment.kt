...
open class CellularFragment(layoutRes: Int = R.layout.fragment_cellular) : Fragment(layoutRes) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnCalibrate?.setOnClickListener {
            ...
            showStartFloorDialog { startFloor ->
                ...
                (btnCalibrate as? TextView)?.text = "Floor $startFloor"
                // Barometer
    ...
    private fun updateCellularInfo() {
        ...
        if (mainActivity.isRecordingCsv && servingFound) {
            ...
            row.add(dataState)
            row.add(
                if (techCsv == "LTE" || techCsv == "LTE_CA") rsrpStr
                else if (techCsv == "NR") nrssRsrp
                else ""
            )
            row.add(techCsv)
            row.add(mcc)
            ...
            row.add(nrPci)
            row.add(nrArfcnStr)
            row.add(if (techCsv == "LTE" || techCsv == "LTE_CA") rsrpStr else "")
            row.add(if (techCsv == "LTE" || techCsv == "LTE_CA") rsrqStr else "")
            row.add(rssnrStr)
            row.add(nrssRsrp)
            row.add(nrssRsrq)
            row.add(nrssSinr)
